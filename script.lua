...
-- Анти-кик за WalkSpeed и JumpPower
spawn(function()
    while task.wait(5)do
        pcall(function()
            if p.Character and p.Character:FindFirstChild("Humanoid")then
                local hr=p.Character.Humanoid
                local oldWS,oldJP=hr.WalkSpeed,hr.JumpPower
                task.wait(.1)
                hr.WalkSpeed=oldWS;hr.JumpPower=oldJP
            end
        end)
    end
end)
...
y=10
local ka=false
toggl(pgs[2],"Kill Aura",function(v)ka=v;spawn(function()while ka do task.wait(.1)pcall(function()for _,pl in ipairs(game.Players:GetPlayers())do if pl~=p and pl.Character and pl.Character:FindFirstChild("Humanoid")and pl.Character.Humanoid.Health>0 then local hr=p.Character and p.Character:FindFirstChild("Humanoid")if hr and hr.Health>0 then pl.Character.Humanoid.Health=0 end end end end)end)end)

-- TOUCH FLING - игрок улетает при касании
local tf=false
...
local fly=false;local flySpeed=50;local flyBody
toggl(pgs[5],"Fly",function(v)fly=v
if v then spawn(function()local hr=p.Character and p.Character:FindFirstChild("Humanoid")if not hr then return end;flyBody=Instance.new("BodyVelocity")flyBody.MaxForce=Vector3.new(1,1,1)*100000;flyBody.Velocity=Vector3.zero;flyBody.Parent=p.Character:FindFirstChild("HumanoidRootPart")or p.Character.Humanoid.RootPart;local bg=Instance.new("BodyGyro",p.Character:FindFirstChild("HumanoidRootPart")or p.Character.Humanoid.RootPart)bg.MaxTorque=Vector3.new(1,1,1)*100000;bg.D=100;bg.P=3000
while fly do task.wait()pcall(function()local vel=Vector3.zero;if uis:IsKeyDown(Enum.KeyCode.W)then vel=vel+workspace.CurrentCamera.CFrame.LookVector end;if uis:IsKeyDown(Enum.KeyCode.S)then vel=vel-workspace.CurrentCamera.CFrame.LookVector end;if uis:IsKeyDown(Enum.KeyCode.A)then vel=vel-workspace.CurrentCamera.CFrame.RightVector end;if uis:IsKeyDown(Enum.KeyCode.D)then vel=vel+workspace.CurrentCamera.CFrame.RightVector end;if uis:IsKeyDown(Enum.KeyCode.Space)then vel=vel+Vector3.new(0,1,0)end;if uis:IsKeyDown(Enum.KeyCode.LeftShift)then vel=vel-Vector3.new(0,1,0)end;if flyBody then flyBody.Velocity=vel*flySpeed end;bg.CFrame=workspace.CurrentCamera.CFrame end)end;if flyBody then flyBody:Destroy()end;bg:Destroy()end)else if flyBody then flyBody:Destroy()flyBody=nil end end end)
